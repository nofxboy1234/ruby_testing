...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:green_tea) { described_class.new('green tea', 16) }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(green_tea.type).to eq('green tea')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(green_tea.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:milk) { described_class.new('milk', 19) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(milk).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:apple_juice) do 
        described_class.new('apple juice', 12)
      end

      # remove the 'x' before running this test
      it 'is not full' do
        expect(apple_juice).not_to be_full
      end
    end
  end
end
