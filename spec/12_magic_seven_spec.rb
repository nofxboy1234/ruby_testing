...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when previous_step is 12 and random_number is 3' do
      subject(:random_number_3) { described_class.new(3) }
      
      it 'returns 9' do
        previous_step = 12
        result = random_number_3.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 5' do
      subject(:random_number_5) { described_class.new(5) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_number_5.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 22' do
      subject(:random_number_22) { described_class.new(22) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_number_22.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 999' do
      subject(:random_number_999) { described_class.new(999) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_number_999.play
        expect(result).to eq(7)
      end
    end
  end
